...
def get_dataset(dataset, data_dir, transform, train=True, needDoubleTransformImage=True, needLabel=True, download=True, debug_subset_size=None):
    if dataset == 'mnist':
        ...
        dataset = torchvision.datasets.CIFAR10("./", train=train, transform=transform, download=download)
        ...
        # dataset = RandomDataset(train=train, transform=transform)
        dataset = FoodDataset(train=train, tfm=transform, needDoubleTransformImage=needDoubleTransformImage, needLabel=needLabel)
    else:
